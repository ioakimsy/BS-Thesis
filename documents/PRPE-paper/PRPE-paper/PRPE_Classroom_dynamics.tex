...
%    An older paper by Tobias~\cite{tobias1990they} suggests that this could be because of the shift of focus of PI from competition and skill performance to cooperative learning and conceptual understanding.
    ...
    The dynamics involved during PI is complex resulting from the interacting components during the peer discussion.
    Existing mathematical models for this are either predictive such as with the case of the artificial neural network (ANN) models~\cite{roxas2010seating} or lack the spatial aspect of the process such as utilizing differential equations~\cite{pritchard2008mathematical,nitta2019mathematical} and those surveyed in \citet{nitta2019mathematical}.
...
% The artificial neural network (ANN) model by \citet{roxas2010seating} used actual assessment results to train a neural network to map student interactions in PI classrooms. 
    ...
    Their study revealed that an optimal seating arrangement for students under PI methods based on their final aptitude.
    In their paper, the measure of students' improvement was calculated via the Hake gain as follows~\cite{hake1998}.
    \begin{equation}
        \label{eq:hake-gain}
    ...
    The results of their study show that the outer corner seating arrangement (SA) performed the best using both the Hake gain~(Eq.~\ref{eq:hake-gain}), followed by inner corner, then random, then center (see Fig.~\ref{fig:PI-SAs} for SA visualizations).
...
%%
    ...
    \citet{nitta2019mathematical} also presented their own model of PI:
%
    \begin{equation}
    ...
    \end{equation}
%
    where $\rho_1$ and $\rho_2$ are the normalized number of students who answered the first and second MCQ correctly respectively. Combining Equation~\ref{eq:nitta_model} with their formulation of gain similar in form to Equation~\ref{eq:hake-gain}, the gain is equal to $\rho_2$.
%
    \begin{equation}
        \label{eq:nitta_gain}
        g = \rho_2
    ...
    There are others whose gain are derived from their own models like that of \citet{pritchard2008mathematical} where the equation is much more complicated.

    Despite the diversity of the models that we found, current models of PI lack some aspects of PI that we would like to incorporate together like seating arrangements, students' learning rate, and aptitude heterogeneity.
    
\section{A probabilistic cellular automata model}

    Here, we show that a probabilistic cellular automata (PCA) model can be used to study the spatiotemporal dynamics of both PI and traditional instruction when incorporating these different aspects into the model.
