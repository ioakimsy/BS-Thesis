...
\section{Comparison with available data}
    ...
    \begin{figure}[htbp]
        ...
        \textcolor{red}{Pink diamonds represent data points from Roxas' work~\cite{roxas2010seating} for comparison.
        The darker data points mean that more points overlap on those coordinates.
        The corresponding best-fit quadratic function for Roxas' data are shown in pink dashed lines with equation $f_t = 0.14 f_{t-1}^2 + 0.68 f_{t-1} + 0.13$.
        For Figure \ref{fig:return_map_unfav}, the fit for TI is $f_t = 0.97 f_{t-1} + 0.029$ and for PI is $f_t = -0.033 f^2_{t-1} + 1.033 f_{t-1}$.
        For Figure \ref{fig:return_map_fav}, the fit for TI is $f_t = 0.548 f_{t-1} + 0.452$ and for PI is $f_t = -0.605 f^2_{t-1} + 1.595 f_{t-1} + 0.019$.}
    ...
    We find that a quadratic function fits the return map for PI mode such that
    \begin{equation}
        h(f) = a_0 + [1 - (a_0+a_2)]f + a_2f^2 \;{\rm [PI \; mode]}
    \end{equation}
    for some parameters $a_i$ ($i=1,2$) with the condition that $h(1)=1$.
    The TI mode for both cases always result to a linear trend in $h(f)$ such that
    \begin{equation}
    ...
    \textcolor{red}{
    Furthermore, our simulation results exhibit consistency with the empirical data reported by Roxas~\cite{roxas2010seating}, as illustrated in Figure~\ref{fig:Return map}.
    Roxas' dataset, consisting of pre- and post-test scores from fourteen PI classrooms, spans a broad range of values.
    The variety in the data can be attributed to many factors that affect classroom learning dynamics such as instructional strategies which may affect the dominance of the effects of PI and TI.
    The timing of pre- and post-tests and the nature of the assessment items can also influence the observed outcomes.
    We note, however, that Roxas' dataset includes instances of score decreases from pre- to post-test---a realistic effect not captured by our model.
    }

    \textcolor{red}{
    A quantitative comparison highlights this trend.
    We obtain curvature coefficients for $f_{t-1}^2$ of $a_2 = 0.14$ for Roxas, $a_2 = -0.033$ for our unfavorable case, $a_2 = -0.605$ for our favorable case, and $a_2 = -1$ for Nitta.
