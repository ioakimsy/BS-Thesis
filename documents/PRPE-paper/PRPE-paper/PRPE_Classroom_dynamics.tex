...
\begin{abstract}
    ...
    In this study, we model classroom learning as a probabilistic cellular automata model 
    and investigate the effects of different factors such as seating arrangements, class size $N$, average learning rate ${\lambda}$, and heterogeneity $\delta{\lambda}$ on the overall class learning efficiency.
    The learning efficiency of the class is measured in terms of the class learning curve {using the total learning time $t_{\rm max}$}.
    PI mode appears to be more applicable with heterogeneous classes (high in-class learning rate difference $\delta{\lambda}$).
    ...
    We show that the class learning curves of highly heterogeneous classes with TI mode have two stages of learning: a fast initial stage and a slow final stage.
    The slow final stage class learning rate is due to the late learners with lower $\lambda$-values.
    Such curves are absent with PI that instead consistently shifts uniformly.
    Our simplified model shows that PI mode {performs better than, or at least similar to,} traditional instruction.
    ...
    We also introduce a class-level estimation of the Hake gain $g$ based on a fitting recurrence function of the simulation data return map.
    Our approach gives consistent results with real-world observations for PI modes and provides opportunities for empirical determination of class-level gains.
\end{abstract}
...
%    Peer Instruction (PI) as a method of instruction aims to address this issue.
    ...
    The method's effectivity has been shown not only in high school physics settings, but also in other disciplines and higher education settings~\cite{johnson2008active,fagen2002peer}.
...
%    An older paper by Tobias~\cite{tobias1990they} suggests that this could be because of the shift of focus of PI from competition and skill performance to cooperative learning and conceptual understanding.
    ...
    The dynamics of the learning process involved during PI is complex resulting from the interaction during the peer discussion stage.
    Existing mathematical models for this are either predictive such as with the case of the artificial neural network (ANN) models~\cite{roxas2010seating,monterola2009} or lack the spatial aspect of the process such as utilizing differential equations~\cite{pritchard2008mathematical,nitta2019mathematical}.
...
%     Comparing their equations to data, they concluded that these metrics and equations roughly agree with the data and could give us insights on the learning dynamics of the classroom.
    Complementing these frameworks, actual classroom assessment results have also been used to model learning under PI. \citet{roxas2010seating} and \citet{monterola2009}, for instance, trained ANN models to predict students’ second assessment scores based on seating arrangements.
    % Actual assessment results by \citet{roxas2010seating,monterola2009} is used to train their ANN to predict expected second assessment scores of students depending on seating arrangements.
% The artificial neural network (ANN) model by \citet{roxas2010seating} used actual assessment results to train a neural network to map student interactions in PI classrooms. 
    ...
    The study revealed that an optimal seating arrangement exists for students under PI methods based on their final aptitude.
    The measure of each students' improvement was calculated via the Hake gain~\cite{hake1998} as 
    %%
    \begin{equation}\label{eq:hake-gain}
        g \equiv (S_2 - S_1) / (1 - S_1)
    \end{equation}
    %%
    where $S_1$ and $S_2$ are scores for the first and second assessment, respectively.
    ...
    \citet{nitta2019mathematical} presented a model of PI learning that relates the probabilities that the students answer the MCQ correctly (effectively, post- and pre-test).
    ...
    This model considers other factors that may affect the learning process such as knowledge before instruction and learning retention.
...
\section{Cellular Automata Method}
    ...
    For this pCA model, each cell in the automaton represents a student arranged in such a way each student is adjacent (a CA {\it neighbor}) to four others resulting to the usual rectangular lattice configuration.
    ...
    Such interaction happens with the nearest neighbor unless specific instructions say differently.
    We consider the simple case such that the probability $P_{i,j}$ is dependent also on the states of the nearest neighbor.
    In this case, we have compounded probabilities resulting to
...
\section{Results and Discussion}
    ...
    \subsection{Dynamical difference between TI and PI}
        ...
        Fig.~\ref{fig:comparison size} shows the typical monotonic increase in the fraction of learned students in classes starting with an unlearned state (all students in {\it unlearned} state, except for PI with which we start with four initially learned students).
        ...
        Due to the required percolation between learned and unlearned states, the PI mode exhibits a gradual initial increase in the number of learned students, followed by a sudden sharp rise in the later stage.
        ...
        The line width is proportional to the $N$ used. Other pertinent parameters fixed: $\rho_0=0.5$ and $\delta\!\lambda=0.2$.
        ...
        Fig. ~\ref{fig:comparison size}(b) shows the effect of SAs for the PI mode.
        The inner corner SA outperforms all the other SAs for PI mode since such SA always has the highest rate of and earliest onset of rise in the number of learned students in the class compared to other SAs.
    ...
    \subsection{Transmission and heterogeneity}
        ...
        Fig. ~\ref{fig:comparison rates}(a) shows that the class learning curves belong to the sigmoidal plots except for characteristic slopes and shifts.
        ...
        \caption{
        Different monotonic increase of the total fraction of students in the learned state over iteration number.
        The blue lines represent the PI mode while the orange lines represent the TI mode.
        Unless indicated, the parameters are fixed such that $N=64$, $\rho_0=0.5$, $\delta\lambda=0.2$ (fairly homogenous) and an Inner Corner SA for PI mode.
        ...
        Fig. ~\ref{fig:comparison rates}(b) shows how this level of heterogeneity $\delta\!\lambda$ affects the resulting class learning curves.
        For the TI mode, the high level of heterogeneity at $\delta\!\lambda=0.4$ (students have either $\lambda=0.1$ or $\lambda=0.9$ within the class) introduces a significant slow down midpoint of the initial rise of class learning curve.
        Such slow down is not significant for low heterogeneity.
        This two-stage learning phenomenon arises from the early transition of fast learners in Stage 1, which leaves the slower learners to catch up only during the later lecture iterations in Stage 2.
        ...
        Fig. ~\ref{fig:comparison rates}(c) shows the comparison of such configurations of the two modes over different iteration times but for smaller class size $N=32$ for brevity.
        ...
        From these observations, we can also conclude that a mode using a combination of both TI and PI will benefit from these two phenomenon of inoculation of learning (via broadcast in TI step) and co-learning from peers (via percolation in the PI step).
    ...
    \subsection{Combined effects}
        The combined effects of average transmission probability (learning rate) $\rho_0$ and of the level of heterogeneity $\delta\!\lambda$ is shown in Fig. ~\ref{fig:Params effect summary t} for two class sizes $N=32^2$ (small) and $128^2$ (large).
        ...
        Both Fig. ~\ref{fig:Params effect summary t}(a) and Fig. ~\ref{fig:Params effect summary t}(b) show that as the level of heterogeneity increases, the traditional approach (TI) require longer $t_{\rm max}$ than the peer instruction (PI) especially for small classes.
        The difference becomes smaller for higher $\rho_0$ or for higher average class aptitude (fast learners).
        For large classes, this difference can actually reverse, but only for cases of homogenous, fast learning, large classes.
        Classically, the utilization of class grouping (``sections'' based on aptitude such as honors class) is thus best suited for TI.
        Instructional strategies would actually prefer PI only for cases of inhomogeneous class.
        %%
        \begin{figure}[htbp!]
            \centering
            ...
            %%
            \caption{Time to learn $t_{\rm max}$ as a function of positional learning factor $\rho_0$ and heterogeneity $\delta\lambda$ of PI (inner corner SA) and TI for two different class sizes $N=L^2$ ((a)$L=32$ and (b)$L=128$).
...
\section{Comparison with available data}
    ...
    Compared to Roxas' work~\cite{roxas2010seating,monterola2009} which found the outer corner SA to perform the best, we found the inner corner SA to perform the best.
    ...
    Class-level learning process is usually assessed via the Hake gain $g$ and consider it related to the recurrence relation between successive evaluations $S_1$ and $S_2$ (cf. Equation~\ref{eq:nitta_model}).
    Since $f$ can be used as an approximation for the probability $S$ that the evaluation items are answered correctly by any member of the class, the Hake gain $g$ can be used as a measure of how our CA model compares well with real-world data.
    ...
    The Nitta Model can be considered as a recurrence equation of the item-based probability $S$ of evaluation of the PI mode as shown to be in agreement with real-world experimental data~\cite{nitta2019mathematical}.
    The model being related to the Hake's gain model via ${S}\rightarrow{f}$ as above, can then be used to confirm the consistency of our simulation data with reality.
    
    Nitta's model have been shown to agree with empirical data suggesting that a student who is likely to answer an evaluation item correctly is more likely to answer any other item in the next evaluation.
    The gain function $g$ can be approximated by the resulting function that fits the return map following Eq.~\ref{eq:nitta_model}.
    The return map that plots $f_{t}$ against $f_{t-1}$ can be shown to mimic Eq.~\ref{eq:nitta_model} following the form of the recurrence equation $f_{t} = h(f_{t-1})$.
    The gain $g(S_1)\approx g(f)$ can then be approximated from real-world data (or simulation data) using
    %%
    \begin{equation}\label{eq:gain-from-return-map}
    g(f) = \frac{h(f) - f}{1 - f}
    \end{equation}
    %%
    with $h(f)$ as the fitting recurrence function to the return map.

    %%%
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.47\textwidth]{figures/figure6.png}
        ...
        Pink diamonds represent data points from Roxas' work~\cite{roxas2010seating} for comparison.
        The darker data points mean that more points overlap on those coordinates.
        The corresponding best-fit quadratic function obtained using the Levenberg-Marquardt algorithm for each set of data points are shown in dashed lines in their corresponding color and are discussed in the text.
    ...
    \end{figure}
    %%
    Fig. ~\ref{fig:Return map} shows the $h(f)$ that fits our simulation data for PI of smaller class sizes (Fig. ~\ref{fig:Return map}(b)) approaching the expectations of the model of Nitta.
    We find that a quadratic function fits 
    \footnote{
        $f_t = -0.033 f^2_{t-1} + 1.033 f_{t-1}$ in Fig. ~\ref{fig:Return map}(a). $f_t = -0.605 f^2_{t-1} + 1.595 f_{t-1} + 0.019$ in Fig. ~\ref{fig:Return map}(b)
    }
    the return map for PI mode such that
    \begin{equation}
        h(f) = a_0 + [1 - (a_0+a_2)]f + a_2f^2 \;{\rm [PI \; mode]}
    \end{equation}
    for some parameters $a_i$ ($i=1,2$) with the condition that $h(1)=1$.
    The TI mode for both cases always results to a linear trend 
    \footnote{
        $f_t = 0.97 f_{t-1} + 0.029$ for Fig. ~\ref{fig:Return map}(a) and $f_t = 0.548 f_{t-1} + 0.452$ for Fig. ~\ref{fig:Return map}(b)
    } 
    in $h(f)$ such that
    \begin{equation}
        h(f) = a_0 + (1-a_0)f \;{\rm [TI \;mode]}
    \end{equation}
    for a different parameter $a_0=m$, again forcing the same limit for $f=1$.
    We therefore see that
    \begin{eqnarray}
        g(f) &=& a_0 - a_2f \;{\rm [PI \;mode]} \\
        g(f) &=& m \;{\rm [TI \;mode]}
    \end{eqnarray}
    for PI and TI modes, respectively ($m$ replaced $a_0$ for the linear fit for distinction)
    \footnote{
        For any polynomial function of degree $N$ fitting the return map, the Hake gain takes the general form $g(f)=a_0 - \sum_{n=2}^{N-2}a_n f\cdot\sum_{m=0}^{n-2}f^m$.
    }.
    When compared to Nitta's results~\cite{nitta2019mathematical}, our simulation model shows $a_0{\approx}0$ making $g(f){\propto}f$.
    Based on our model, PI mode results to a linear Hake gain consistent with their results but predicts a relatively constant  gain for TI mode.
    The efficiency (Nitta) of the PI can also be related to the constant $|a_2|$.

    In our simulation results (cf. Fig. ~\ref{fig:Return map}), we find that the PI mode efficiency is lost to high class size giving $|a_2|=0.60$ for $N=32^2$ to $0.03$ for $N=128^2$.
    TI mode gain also declined from $m=0.45$ to $0.029$ with the same increase in class size.
    These results are consistent with the overall assessment based on other dynamical parameters presented above.

    Our results are consistent with the empirical data reported by Roxas~\cite{roxas2010seating}.
    Roxas' dataset, consists of pre- and post-test scores from fourteen PI classrooms and spans a broad range of values.
    Fig. ~\ref{fig:Return map} plots the pre-test scores plotted as $f_{t-1}$ and post-test scores plotted as $f_{t}$.
    The variety in the data can be attributed to many factors that affect classroom learning dynamics such as instructional strategies which may affect the dominance of the effects of TI over PI.
    The timing of pre- and post-tests and the level of precision of the assessment method can also influence the observed outcomes.
    Roxas' dataset includes instances of score decreases from pre- to post-test---a realistic effect not captured by our model.
    We obtain coefficients of the quadratic terms---$a_2 = 0.14$ for Roxas \footnote{
        $f_t = 0.14 f_{t-1}^2 + 0.68 f_{t-1} + 0.13$
    }, $a_2 = -0.033$ for our unfavorable case, $a_2 = -0.605$ for our favorable case, and $a_2 = -1$ for Nitta.
    Taken together, these results indicate that while our PI model does not reproduce all empirical features, it provides a close approximation to the observed classroom dynamics shown in Nitta's model and Roxas' data.

    %! 1. Combination of many factors
    %! 2. May be dominated by TI
    %! 3. Effects of PI
    %! 4. Uncertainty regarding pre-test and post-test (points below y=x)
...
\section{Summary and Conclusion}
    ...
    Through this model, we investigated how different factors affecting classroom learning dynamics for both traditional instruction (TI) and peer instruction (PI).
    ...
    Larger class size ($N$), higher level of heterogeneity ($\delta\!\lambda$), and higher learning transmission efficiency ($\rho_0$) are not advantageous to both TI and PI modes of classroom learning.
    TI mode though is not as affected by class size compared to PI while heterogeneity has lesser impact with PI mode.
    Other simulation observations are discussed consistent with the real-world experience on these modes of classroom instruction.

    Consistent with known empirical real-world data results, our simulation data is able to capture theoretical predictions of the possible forms of Hake gains for both TI and PI modes of the model.
    We presented an empirical determination of the effective Hake gain of the mode of instruction based on a return map data from class level pre- and post-test results.
    This approach will provide future studies better way of determining class-level Hake gains instead of solely based on individual averages.

    While our model is simplistic, we have shown that it be a good approximation of the real world and higher-order effects be easily incorporated (e.g. anisotropy of learning in PI, more levels of learner types)
    ...
    Additionally, transitioning to a continuous-state model would allow incorporating more real-world phenomena like aptitude similarity and orientation effects during co-learning.
