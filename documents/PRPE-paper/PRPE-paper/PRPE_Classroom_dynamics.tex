...
\section{Comparison with available data}
    ...
    \end{figure}
    %%
    Figure~\ref{fig:Return map} shows the $h(f)$ that fits our simulation data for PI of smaller class sizes (Figure~\ref{fig:Return map}(b)) approaching the expectations of the model of Nitta.
    We find that a quadratic function fits 
    \footnote{
        $f_t = -0.033 f^2_{t-1} + 1.033 f_{t-1}$ in Figure~\ref{fig:Return map}(a). $f_t = -0.605 f^2_{t-1} + 1.595 f_{t-1} + 0.019$ in Figure~\ref{fig:Return map}(b)
    }
    ...
    \footnote{
        $f_t = 0.97 f_{t-1} + 0.029$ for Figure~\ref{fig:Return map}(a) and $f_t = 0.548 f_{t-1} + 0.452$ for Figure~\ref{fig:Return map}(b)
    } 
    in $h(f)$ such that
    \begin{equation}
    ...
    In our simulation results (cf. Figure~\ref{fig:Return map}), we find that the PI mode efficiency is lost to high class size giving $|a_2|=0.60$ for $N=32^2$ to $0.03$ for $N=128^2$.
    ...
    Figure~\ref{fig:Return map} plots the pre-test scores plotted as $f_{t-1}$ and post-test scores plotted as $f_{t}$.
