...
\begin{abstract}
    ...
    In this study, we model the \textcolor{red}{transfer of knowledge} within classes of students as a probabilistic cellular automata model 
...
% The artificial neural network (ANN) model by \citet{roxas2010seating} used actual assessment results to train a neural network to map student interactions in PI classrooms. 
    Using this neural network, they were also able to characterize \textcolor{red}{information transfer} and investigate the effects of group homogeneity. 
    ...
    \end{equation}
    %%
    \textcolor{red}{Other models provide Hake gain expressions} (e.g. \citet{pritchard2008mathematical}) resulting to more complex formulation for the function $g(S_1)$.
...
\section{Cellular Automata Method}
    ...
    \end{equation}
    %%
    with $\rho_{\delta i,\delta j}\in[0,1]$ as the potential dependence of \textcolor{red}{learning transfer} efficiency on the relative position $(\delta i, \delta j)$ of the source of learning (inspiration, input, interaction, discussion, etc.) with respect to the student position $(i,j)$~\cite{dong2021influence}.
