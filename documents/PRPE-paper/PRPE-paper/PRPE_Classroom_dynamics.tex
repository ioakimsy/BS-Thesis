...
\section{Comparison with available data}
    ...
    The return map that plots $f_{t}$ against $f_{t-1}$ can be shown to mimic Eq.~\ref{eq:nitta_model} and hint the form of the gain $g(S_1)\approx g(f)$, even for real-world data, not just for numerical simulation.

    {\color{red} Modify from hereon.}
    Figure~\ref{fig:Return map} shows our data for PI approaches their model --- this is expected since their model is for PI and not TI.
