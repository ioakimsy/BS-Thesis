...
\begin{abstract}
    ...
    Such curves are absent with PI that instead consistently shifts uniformly depending on the other factors.
    ...
    We also introduce a class-level estimation of the Hake gain $g$ based on a fitting recurrence functino of the simulation data return map.
...
% The artificial neural network (ANN) model by \citet{roxas2010seating} used actual assessment results to train a neural network to map student interactions in PI classrooms. 
    ...
    \end{equation}
    %%
    Other models provide Hake gain expressions (e.g. \citet{pritchard2008mathematical,eq:pritchard_connectedness_gain})resulting to more complex formulation for the function $g(S_1)$.
...
\section{Comparison with available data}
    ...
    Nitta's model have been shown to agree with empirical data suggesting that a student who is likely to answer an evaluation item correctly is more likely to answer any other item in the next evalution.
    ...
    The gain $g(S_1)\approx g(f)$ can then be approximated from real-worl data (or simulation data) using
    %%
    ...
    \begin{figure}[htbp]
        \centering
        \subfigure[$L=128,\space\rho_0=0.1,\space\lambda=0.5\pm0.4$]{\label{fig:return_map_unfav}\includegraphics[width=0.47\textwidth]{figures/2D-BPCAIH-analysis/return plots/return-map-128-0.1-0.5-0.4-data.png}}
        \subfigure[$L=32,\space\rho_0=0.9,\space\lambda=0.5\pm0.0$]{\label{fig:return_map_fav}\includegraphics[width=0.47\textwidth]{figures/2D-BPCAIH-analysis/return plots/return-map-32-0.9-0.5-0.0-data.png}}
    ...
    \end{figure}
    %%
    Figure~\ref{fig:Return map} shows the $h(f)$ that fits our simulation data for PI of smaller class sizes (Figure~\ref{fig:return_map_fav}) approaching the expectations of the model of Nitta.
    We find that a quadratic function fits 
    \footnote{
        $f_t = -0.033 f^2_{t-1} + 1.033 f_{t-1}$ in Figure~\ref{fig:return_map_unfav}. $f_t = -0.605 f^2_{t-1} + 1.595 f_{t-1} + 0.019$ in Figure~\ref{fig:return_map_fav}
    }
    ...
    The TI mode for both cases always result to a linear trend 
    \footnote{
        $f_t = 0.97 f_{t-1} + 0.029$ for Figure~\ref{fig:return_map_unfav} and $f_t = 0.548 f_{t-1} + 0.452$ for Figure~\ref{fig:return_map_fav}
    } 
    in $h(f)$ such that
    \begin{equation}
...
\section{Summary and Conclusion}
    ...
    Through this model, investigated how different factors affecting classroom learning dynamics for both traditional instruction (TI) and peer instruction (PI).
