...
begin
    ...
    for seat_config in SAs, ρ₀ in Ρs
        ...
        band!(ax, sizes #.^ 2
                , maximum(ttl_maxes, dims = 1)[1], minimum(ttl_mins, dims = 1)[1],
                color = (color[color_count], rho_alpha_dict[ρ₀]/4)
            )
    ...
    save("./output/2D-Binary-PCA-IH/analysis/plots/l-t-plots/l-t-plot-ribbon.png", fig)
    ...
    for ρ₀ in Ρs
        ...
        for seat_config in SAs
            for δλ in δλs
                ...
                lines!(ax, subset.class_size #.^ 2
                    , Measurements.value.(subset.ttl),
                    label = "$(SA_label_dict[seat_config]), δλ=0.5±$δλ ",
                    linestyle = δλ_ls_dict[δλ],
                    color = (color[SA_color_dict[seat_config]], rho_alpha_dict[ρ₀])
                )


                band!(ax, subset.class_size #.^ 2
                    , ttl_max, ttl_min,
                    color = (color[SA_color_dict[seat_config]], rho_alpha_dict[ρ₀]/4)
                )
            end
        end
        ...
        savepath = "./output/2D-Binary-PCA-IH/analysis/plots/l-t-plots/"
        filename = "l-t-plot-$ρ₀"
