    ...
    for trial in 1:n_trials
        for seat_config in seat_configs, λ₀ in Λs, class_size in sizes#, trial in 1:n_trials
            ...
            if animation == true
                ...
                if seat_config == "random"
                    mp4(anim, "./output/2D-Binary-PCA/random-$(class_size)-$(n_learned)/$(λ₀)/trial_$(trial)/2DBPCA-random-$(class_size)-$(n_learned)-$(λ₀)-animation.mp4", fps = 16);
                else
                    mp4(anim, "./output/2D-Binary-PCA/$(seat_config)-$(class_size)/$(λ₀)/trial_$(trial)/2DBPCA-random-$(class_size)-$(λ₀)-animation.mp4", fps = 16);
                end # animation save end
            end

        end # params end
    end # trial iteration end
end # function end
...
generate_plots(sizes,seat_configs, Λs, steady_state_tolerance, n_trials; n_learned = n_learned, animation = true)